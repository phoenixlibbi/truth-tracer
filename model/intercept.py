...
#   global recording
#   global RECORD
#   RECORD = True  # Set recording mode to True by default
...
#             calibration_frames += process(full_name, image, face_mesh, hands, calibrated, DRAW_LANDMARKS, BPM_CHART, FLIP)
#             calibrated = (calibration_frames >= MAX_FRAMES)
#             if SECOND:
#               process_second(cap2, image, face_mesh, hands)
#               asyncio.get_event_loop().run_until_complete(sendtoFrontend(image))
...
#         if RECORD:
#           RECORDING_FILENAME = full_name + '.avi'
...
#             # process_second(cap2, image, face_mesh, hands)
#           else:
#             calibration_frames += process(full_name, image, face_mesh, hands, calibrated, DRAW_LANDMARKS, BPM_CHART, FLIP, fps)
#             calibrated = (calibration_frames >= MAX_FRAMES)
#             if SECOND:
#               process_second(cap2, image, face_mesh, hands)
...
def main(full_name):
    ...
    # Split full_name into name and option
    if '_' in full_name:
        name, option = full_name.rsplit('_', 1)
    else:
        name, option = full_name, '1'  # Default to option 1 if not provided

    # Initialize variables based on option
    INPUT = 0
    ...
    if option == '1':
        ...
        TELL_MAX_TTL = 30  # Example value, adjust if needed
    elif option == '5':
        TELL_MAX_TTL = 30  # Example value, adjust if needed
    elif option == '6':
        SECOND = 1  # Example value, adjust if needed
    ...
    with mp.solutions.face_mesh.FaceMesh(
        ...
        with mp.solutions.hands.Hands(
            ...
            while cap.isOpened():
                ...
                if not success:
                    break
                calibration_frames += process(name, image, face_mesh, hands, calibrated, DRAW_LANDMARKS, BPM_CHART, FLIP, fps)
                calibrated = (calibration_frames >= MAX_FRAMES)
                if SECOND:
                    process_second(cap2, image, face_mesh, hands)
                ret, buffer = cv2.imencode('.jpg', image)
...
# Assuming you have a WebSocket server running on ws://localhost:8765
websocket_server_address = "ws://localhost:8765"

async def sendtoFrontend(image):
    try:
        ...
        print("Error while sending image to frontend:", e)

# Make sure to run this code in an event loop if not already running in an asynchronous environment
# asyncio.get_event_loop().run_until_complete(sendtoFrontend(image))
...
def add_truth_meter(full_name, image, tell_count):
  width = image.shape[1]
  sm = int(width / 64)
  bg = int(width / 3.2)
  logging.debug(f"Image width: {width}, sm: {sm}, bg: {bg}")
  script_directory = os.path.dirname(os.path.abspath(__file__))
  meter_image_path = os.path.join(script_directory, "meter.png")
  meter = cv2.imread(meter_image_path)
  ...
  if tell_count:
    ...
    with open(f"{full_name}_tell.txt", "a") as tell_file:
      # Write tellX value followed by a newline character
      tell_file.write(f"{tellX}\n")
...
def process(full_name, image, face_mesh, hands, calibrated=False, draw=False, bpm_chart=False, flip=False, fps=None,):
  ...
  add_truth_meter(full_name, image, len(tells))
